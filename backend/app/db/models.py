...
class RouteHistory(Base):
    """Saved route analysis — stores full result for re-opening without re-analyzing."""
    ...
    source_format: Mapped[str | None] = mapped_column(String(10))  # GPX, GEOJSON
    is_favorite: Mapped[bool] = mapped_column(Boolean, default=False)

    # Denormalized summary for fast list queries
    mide_global: Mapped[int | None] = mapped_column(Integer)
    total_distance_km: Mapped[float | None] = mapped_column(Float)
    estimated_time_h: Mapped[float | None] = mapped_column(Float)
    total_kcal: Mapped[float | None] = mapped_column(Float)
    elevation_gain_m: Mapped[float | None] = mapped_column(Float)
    risk_max: Mapped[int | None] = mapped_column(Integer)

    # Full analysis result (for re-opening without re-analyzing)
    analysis_json = mapped_column(Text, nullable=False)  # JSONB not used with asyncpg directly
